...
<div class="flex items-start gap-8 w-full my-6 -ml-1.5">
    ...
    <div>
        ...
        <p class="text-sm">
            ...
            Add Product Bibit <span
                class="font-bold text-indigo-600">{{ $item->product_bibit->product_name }}</span>.
        </p>
        <p class="text-xs text-gray-500">{{ $formattedTime }}</p>
        @if ($item->product_bibit->description)
            <div class="p-4 mt-4 text-sm border border-gray-200 rounded shadow-lg">
                <p>{!! $item->product_bibit->description !!}</p>
            </div>
        @endif
    </div>
