<x-app-layout>
    {{-- start::Alert --}}
    @include('product.partials.alert')
    {{-- end::Alert --}}
    <div>
        <!-- Page Header -->
        <div class="flex items-center justify-between mb-8">
            <div>
                <ol class="flex gap-3 text-sm">
                    ...
                    <li class="text-indigo-500 breadcrumb-item active" aria-current="page">Product</li>
                </ol>
            </div>
        </div>
            ...
            <div class="px-2 mb-12" x-data="{ openDelete: false, petugasId: 0 }">
                <div class="flex items-center justify-between">
                    <div class="flex items-center justify-between gap-3">
                        <h4 class="text-base font-semibold">Product Bibit</h4>
                        {{-- tambah can --}}
                        <a href="{{ route(name: 'product.create', absolute: false) }}" class="px-4 py-2 text-sm text-indigo-500 transition duration-150 rounded bg-indigo-50 hover:bg-indigo-500 hover:text-white">
                            New Bibit
                        </a>
                        {{-- tambah encan --}}
                    </div>
                    @include('product.partials.filter-form', [
                        'search' => $search,
                            ...
                            <x-table-row>
                                <x-table-data>
                                    <div class="flex items-center gap-3">
                                        <img src="{{ $item->media->last() != null ? $item->media->last()->getUrl() : Vite::asset('resources/images/default-product.jpg'); }}" width="42" height="42" alt="" class="rounded full">
                                    </div>
                                </x-table-data>
                                ...
                                <x-table-actions>
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route(name: 'product.edit', parameters: ['product' => $item->id], absolute: false) }}" class="px-4 py-2 text-xs font-medium text-yellow-500 transition duration-150 rounded bg-yellow-50 hover:bg-yellow-500 hover:text-white">
                                        ...
                                        {{-- <a href="{{ route(name: 'product.show', parameters: ['product' => $item->id], absolute: false) }}" class="px-4 py-2 text-xs font-medium text-indigo-500 transition duration-150 rounded bg-indigo-50 hover:bg-indigo-500 hover:text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18">
                                                <path
