<aside :class="menuOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-10 w-64 overflow-y-auto transition duration-300 bg-slate-500 lg:translate-x-0 lg:inset-0 custom-scrollbar">
    ...
    <nav class="py-10 custom-scrollbar">
      <!-- start::Menu link -->
      <a x-data="{ linkHover: false }" @mouseover="linkHover = true" @mouseleave="linkHover = false" href="{{ route(name: 'dashboard', absolute: false) }}" class="flex items-center px-6 py-3 text-gray-300 transition duration-200 cursor-pointer hover:bg-black hover:bg-opacity-30">
