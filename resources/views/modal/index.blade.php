        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <ol class="flex gap-3 text-sm">
                    ...
                    <li class="text-indigo-500 breadcrumb-item active" aria-current="page">Modal</li>
                </ol>
            </div>
        </div>
                ...
                <div class="p-5 mb-12 bg-white rounded">
                    <div class="px-2 mb-12" x-data="{ openDelete: false, modalId: 0 }">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center justify-between gap-3">
                                <h4 class="text-base font-semibold">Modal</h4>
                                {{-- tambah can --}}
                                <a href="{{ route(name: 'modal.create', absolute: false) }}" class="px-4 py-2 text-sm text-indigo-500 transition duration-150 rounded bg-indigo-50 hover:bg-indigo-500 hover:text-white">
                                    New Modal
                                </a>
                                {{-- tambah encan --}}
                            </div>
                            @include('modal.partials.filter-form', [
                                'search' => $search,
                                        ...
                                        <x-table-row>
                                            ...
                                            {{-- <x-table-actions>
                                                <div class="flex items-center justify-center gap-2">
                                                    ...
                                                    <a href="{{ route(name: 'modal.show', parameters: ['modal' => $item->id], absolute: false) }}" class="px-4 py-2 text-xs font-medium text-indigo-500 transition duration-150 rounded bg-indigo-50 hover:bg-indigo-500 hover:text-white">
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18">
                                                            <path
                                                    ...
                                                    <button class="px-4 py-2 text-xs font-medium text-red-500 transition duration-150 rounded bg-red-50 hover:bg-red-500 hover:text-white" @click="openDelete = true; modalId = @js($item->id)">
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18">
                                                            <path
                        ...
                        <div x-show="openDelete" class="fixed top-0 left-0 right-0 z-50 flex items-center justify-center w-full overflow-x-hidden overflow-y-auto bg-black bg-opacity-80 md:inset-0 h-modal md:h-full" x-transition.opacity x-transition:enter.duration.100ms x-transition:leave.duration.300ms x-cloak>
                            <div class="relative w-full h-full max-w-md p-4 md:h-auto" @click.away="openDelete = false; modalId: 0">
                                {{-- Modal Content --}}
                                <div class="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
                                    <button type="button" @click="openDelete = false; modalId: 0" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal">
                                    ...
                                    <div class="flex items-center justify-center space-x-4">
                                        <form :action="route('modal.destroy', {'modal': modalId}, false)" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <button @click="openDelete = false; modalId: 0"
